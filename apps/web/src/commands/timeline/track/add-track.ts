...
function buildOverlayTrackState({
	...
	const newTrack =
		trackType === "video"
				? buildEmptyTrack({ id: trackId, type: "video", ...(name ? { name } : {}) })
				: trackType === "text"
					? buildEmptyTrack({ id: trackId, type: "text", ...(name ? { name } : {}) })
					: trackType === "graphic"
						? buildEmptyTrack({ id: trackId, type: "graphic", ...(name ? { name } : {}) })
						: buildEmptyTrack({ id: trackId, type: "effect", ...(name ? { name } : {}) });
	return {
		...tracks,
		overlay: [
