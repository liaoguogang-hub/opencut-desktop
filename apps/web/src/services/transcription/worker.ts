...
// Electron context has no CORS restrictions, so we can fetch the model
// mirror directly. Default to hf-mirror (reliable from mainland China) and
// fall back to huggingface.co if a model isn't found there.
//
// `allowLocalModels` lets transformers.js serve pre-baked models from
// `apps/web/public/models/...` (served at `/models/...`) before hitting
// the network — see `scripts/download-models.sh` for the helper.
//
// Note: the ORT WASM files are loaded from a separate URL independent of
// `remoteHost` — transformers.js v3.8.1 already defaults this to
// `https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1/dist/`,
// which is reachable from mainland China, so we leave it untouched.
...
async function handleInit({ modelId }: { modelId: string }) {
	...
	try {
		console.log("[transcription-worker] init start", modelId);
		...
		console.log(
			"[transcription-worker] pipeline ready",
			modelId,
			"files cached:",
			fileBytes.size,
		);
...
async function handleTranscribe({
	...
	if (!transcriber) {
		console.error("[transcription-worker] transcribe called before init");
		self.postMessage({
	...
	console.log(
		"[transcription-worker] transcribe start",
		"audio seconds:",
		(audio.length / 16000).toFixed(2),
		"language:",
		language,
	);
	cancelled = false;
